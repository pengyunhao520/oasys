...
public class ProcedureController {
	...
	//红头文件发文申请
	@RequestMapping("documentregistration")
	public String documentRegistration(Model model, @SessionAttribute("userId") Long userId,HttpServletRequest request,
						   @RequestParam(value = "page", defaultValue = "0") int page,
						   @RequestParam(value = "size", defaultValue = "10") int size){
		//查找类型
		List<SystemTypeList> outtype=tydao.findByTypeModel("document_registration");
		proservice.index6(model, userId, page, size);
		model.addAttribute("outtype", outtype);
		return "filesend/documentregistration";
	}
	/**
	 * 红头文件申请表单接收
	 * @param model
	 ...
	 * @throws IllegalStateException
	 */
	@RequestMapping(value = "eveceee")
	public String documentRegistrationDao(@RequestParam("enclosure")MultipartFile enclosure,HttpServletRequest req,@Valid DocumentRegistration dr,BindingResult br,
					   @SessionAttribute("userId") Long userId) throws IllegalStateException, IOException{

		Date date = new Date();
		dr.setDate(date);
		User lu=udao.findOne(userId);//申请人
		User shen=udao.findByUserName(dr.getApprover());//审核人
		System.out.println("++++++++++++++++++++");
		...
		//Long userid=shen.getUserId();//审核人userid
		String val=req.getParameter("val");
		if(dr.getApprover()!=null){
			//set主表
			//ProcessList pro=eve.getProId();
			//proservice.index5(pro, val, lu, filePath,shen.getUserName());
			ProcessList pro=dr.getProId();
			pro.setDeeply(22L);
	...
	public String changeUnit(@RequestParam(value = "unit",required=false)Long id,Model model){
		if(id!=null){
			DocumentRegistration dr = drdao.findOne(id);
			model.addAttribute("dr",dr);
		}
		return "process/documentRegistration";
	}*/
